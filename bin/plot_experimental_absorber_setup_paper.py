...
def main(W=0.05, L=25, config=1, plot_phase=None, plot=False, save_plot=False, threshold_left=None, threshold_right=None):
    ...
    ax33.set_xticklabels(["".format(t) for t in snapshots_delta_values])
    for tick in ax33.get_xaxis().get_major_ticks():
        tick.set_pad(0.0)
    ax33.grid(True, lw=1.)

    for a in (ax, ax2, ax3, ax33):
        a.spines['right'].set_visible(False)
        a.spines['top'].set_visible(False)
        a.get_xaxis().set_tick_params(direction='out')
        a.get_yaxis().set_tick_params(direction='out')
        a.tick_params(axis='both', which='minor', bottom='off',
                      left='off', right='off', top='off')
        a.tick_params(axis='both', which='major', bottom='on',
